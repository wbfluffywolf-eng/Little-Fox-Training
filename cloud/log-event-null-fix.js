...
function isLogForm(form) {
  return form?.id === "logForm" ||
    (form?.querySelector("[name='changed_at']") && form?.querySelector("[name='diaper_id']") && form?.querySelector("[name='subcategory']"));
}

async function writableHouseholdForCurrentUser() {
...
document.addEventListener("submit", async event => {
  ...
  try {
    const { householdId, userId } = await writableHouseholdForCurrentUser();
    ...
    console.error("Log save fix failed", error);
