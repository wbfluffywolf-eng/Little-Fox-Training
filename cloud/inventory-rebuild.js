...
function catalogType(productType) {
  ...
  if (lower.includes("shell") || lower.includes("kit") || lower.includes("diaper") || lower.includes("brief")) return "cloth";
...
function rowPresets(row) {
  const item_type = catalogType(row.product_type || row.category || row.type_style);
  const brand = row.brand?.trim();
  const style = row.product_name?.trim();
  if (!brand || !style) return [];

  const presets = [];
  const sourceSizes = row.size?.trim()
    ? [row.size.trim()]
    : String(row.sizes || "One size").split(";").map(size => size.trim()).filter(Boolean);
  ...
  sourceSizes.forEach(size => {
    ...
    if (unitPrice) presets.push({ item_type, brand, style, size, stock_count: 1, purchase_price: unitPrice });
  });
  return presets;
}
...
async function loadPresetCatalog() {
  ...
  catalogLoadPromise = Promise.all(catalogFiles.map(file =>
    fetch(file, { cache: "no-store" })
      .then(response => response.ok ? response.text() : "")
      .catch(() => "")
  ))
    .then(texts => {
      const rows = texts.flatMap(text => parseCsv(text).flatMap(rowPresets));
      const seen = new Set();
...
async function loadInventoryContext() {
  ...
  const [diapers, logs] = await Promise.all([
    ...
    supabase.from("logs").select("diaper_id, insert_ids, changed_at, happened_at").eq("household_id", household.id).limit(350)
  ]);
...
document.addEventListener("click", event => {
  if (event.target.closest('[data-tab="inventory"]')) setTimeout(() => rebuildInventory(), 100);
});

[0, 500, 1500].forEach(delay => setTimeout(() => rebuildInventory(), delay));
