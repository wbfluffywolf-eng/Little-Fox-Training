...
async function loadContext() {
  ...
  const { data: memberships } = await supabase
    ...
    .eq("status", "active");
  const member = (memberships || []).find(row => row.role === "owner" || row.households?.owner_id === session.user.id)
    || memberships?.[0];
  ...
  const [diapers, logs, expenses] = await Promise.all([
    supabase.from("diapers").select("*").eq("household_id", household.id).order("created_at", { ascending: false }).limit(400),
    supabase.from("logs").select("*, diapers(brand, style, size, item_type, purchase_price)").eq("household_id", household.id).order("happened_at", { ascending: false }).limit(350),
    supabase.from("expenses").select("*").eq("household_id", household.id).order("expense_date", { ascending: false }).limit(200)
  ]);
...
function dirtyCount(ctx, id) {
  ...
  return ctx.logs.filter(log => logUsesItem(log, id) && new Date(log.changed_at || log.happened_at).getTime() >= cutoff).length;
}

function cleanCount(ctx, item) {
  return Math.max(0, Number(item.clean_count ?? item.stock_count ?? 0) - dirtyCount(ctx, item.id));
}

function option(item, ctx) {
  const clean = ["cloth", "cloth_insert", "underpad"].includes(item.item_type) ? ` - ${cleanCount(ctx, item)} clean` : "";
  return `<option value="${item.id}" ${clean.startsWith(" - 0") ? "disabled" : ""}>${esc(item.brand)} ${esc(item.style)} (${esc(item.size || itemTypes[item.item_type])}${esc(clean)})</option>`;
}
...
function logFields(ctx) {
  ...
  return `
    <div class="form-grid">
      <label>What happened<div class="pill-row"><label><span><input type="checkbox" name="event_wet" checked> Wet</span></label><label><span><input type="checkbox" name="event_messed"> Messed</span></label><label><span><input type="checkbox" name="event_dry"> Dry</span></label></div></label>
      ...
      <label>Cloth diaper<select name="diaper_id" required>${shells.map(item => option(item, ctx)).join("")}</select></label>
      <label>Inserts / boosters<select name="insert_ids" multiple size="5">${inserts.map(item => option(item, ctx)).join("")}</select></label>
      <label>Subcategory<select name="subcategory">${subcategories.map(([id, label]) => `<option value="${id}">${label}</option>`).join("")}</select></label>
...
function itemCard(ctx, item) {
  const clean = cleanCount(ctx, item);
  const dirty = dirtyCount(ctx, item.id);
  const uses = ctx.logs.filter(log => logUsesItem(log, item.id));
  const leaks = uses.filter(log => log.leaked).length;
  const messes = uses.filter(log => log.event === "messed").length;
...
async function saveClothWear(event, ctx) {
  ...
  if (!events.length || data.has("event_dry")) events.splice(0, events.length, "dry");
  const row = {
  ...
  const rows = events.map(eventName => ({
    ...row,
    event: eventName,
    notes: events.length > 1 ? `${row.notes}${row.notes ? " " : ""}(same change: wet and messed)` : row.notes
  }));
  const { error } = await supabase.from("logs").insert(rows);
  if (error) {
    if (button) {
      button.disabled = false;
      button.textContent = "Save Cloth Wear";
    }
    return toast(`Cloth wear could not save: ${error.message}`);
  }
...
async function renderClothPage() {
  ...
  view.innerHTML = `
    <div class="grid three">${stat("Clean Cloth", clean, "ready to wear")}${stat("Dirty Cloth", dirty, "returns after 48 hours")}${stat("Cloth Wears", wears, "saved changes")}</div>
    ...
    <section class="grid two" style="margin-top:14px">
      <article class="card"><h3>Cloth Diapers</h3><div class="list" style="margin-top:12px">${shells.map(item => itemCard(ctx, item)).join("") || `<div class="empty">No cloth diapers yet.</div>`}</div></article>
      <article class="card"><h3>Cloth Inserts</h3><div class="list" style="margin-top:12px">${inserts.map(item => itemCard(ctx, item)).join("") || `<div class="empty">No cloth inserts yet.</div>`}</div></article>
      <article class="card"><h3>Recent Cloth Wears</h3><div class="list" style="margin-top:12px">${ctx.logs.filter(log => cloth.some(item => logUsesItem(log, item.id))).slice(0, 12).map(logItem).join("") || `<div class="empty">No cloth wears logged yet.</div>`}</div></article>
      <article class="card"><h3>Cloth Cost Per Wear</h3><div class="list" style="margin-top:12px">${cloth.map(item => itemCard(ctx, item)).join("") || `<div class="empty">No cloth cost data yet.</div>`}</div></article>
    </section>
  `;
  document.getElementById("enhancedClothWearForm")?.addEventListener("submit", event => saveClothWear(event, ctx));
}

async function importStarterItems(ctx) {
  const keys = new Set(ctx.diapers.map(item => `${item.brand}|${item.style}|${item.size}|${item.item_type}`.toLowerCase()));
  const rows = starterItems
    .filter(item => !keys.has(`${item.brand}|${item.style}|${item.size}|${item.item_type}`.toLowerCase()))
    .map(item => ({ ...item, household_id: ctx.household.id }));
  if (!rows.length) return toast("Starter items are already imported.");
  const { error } = await supabase.from("diapers").insert(rows);
  if (error) return toast(`Starter import failed: ${error.message}`);
  toast(`Imported ${rows.length} starter items.`);
  setTimeout(() => location.reload(), 700);
}

async function enhanceInventory() {
  const ctx = await loadContext();
  if (!ctx?.isOwner || document.getElementById("starterImportBtn")) return;
  const view = document.getElementById("view");
  const firstCard = view?.querySelector(".card");
  if (!firstCard) return;
  firstCard.insertAdjacentHTML("afterend", `<article class="card" style="margin-top:14px"><h3>Starter Import</h3><p>Add the saved cloth diapers, inserts, disposables, and supplies list without duplicating existing matching items.</p><button class="btn sky" id="starterImportBtn" type="button">Import Starter Items</button></article>`);
...
let lastPage = "";
async function routeEnhancements() {
  const title = document.querySelector(".topbar h2")?.textContent.trim();
  if (!title || title === lastPage) return;
  lastPage = title;
  if (title === "Cloth Diapers") await renderClothPage();
  if (title === "Inventory") await enhanceInventory();
  if (title === "Trends") await enhanceTrends();
}

document.addEventListener("click", event => {
  if (event.target.closest("[data-tab]")) setTimeout(routeEnhancements, 100);
});

routeEnhancements();
[500, 1500].forEach(delay => setTimeout(routeEnhancements, delay));
